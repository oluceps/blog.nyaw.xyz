...
const Col: Component<ParentProps & { title: string; comment: string }> = (
	...
	return (
		<div
			class="rounded-md hover:shadow-md border border-sprout-300 transition-all duration-200 p-3 mt-3"
			onClick={() =>
				props.title == activeState()
					? setActiveState("")
					: setActiveState(props.title)
			}
		>
			<section>
				<div class="flex justify-start items-center">
					<Icon
						path={plus}
						class={`w-4 h-4 mx-3 my-2 transition-all duration-500 transform-gpu ${isActive() ? "rotate-45" : ""}`}
					/>
					<div
						class={`font-bold flex w-full items-center justify-between mx-3 ${isActive() ? "text-sprout-600" : "text-zinc-500"}`}
					>
						<div>{props.title}</div>
