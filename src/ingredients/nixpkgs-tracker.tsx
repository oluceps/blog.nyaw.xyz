...
const Tracker = () => {
  ...
  const isContain = async (branch: string, commit: string): Promise<boolean> => {
    try {
      const { status } = await api.get(`compare/${branch}...${commit}`).json<any>();
  ...
  const resetBranchStatus = () => branchStatus.forEach((_, v) => {
    ...
    <div class="grid md:grid-cols-2 w-full gap-3">
      <div class="flex flex-col w-full justify-center items-center space-y-4 md:space-y-2">
        <label class="input input-bordered flex items-center gap-2">
          <input type="text" class="grow" placeholder="PR Number" onBeforeInput={e => setQnum(Number.parseInt(e.target.value))} onInput={e => setQnum(Number.parseInt(e.target.value))} />
          <svg
        ...
        <label class="input input-bordered flex items-center gap-2">
          ...
          <input type="password" class="grow token" placeholder="API Token (Optional)" onInput={(e) => setTokenText(e.data!)} />
        </label>
        <button class="btn glass" disabled={!btnStatus()} onClick={async () => await handlePR(qnum()!)}>
          <Show when={loading()}>
      ...
      <div class="flex flex-col justify-center items-center">
        <For each={Array.from(branchStatus)}>
          {(k, _) => {
            return (<><div class="flex items-center"><div class={twMerge(`text-sprout-300`, k[1] ? `text-red-400` : `text-slate-300`)}>{k}</div><Show when={k[1]}><TiTick /></Show></div></>)
          }}
        </For>
      </div>
    </div>
