...
export const preprocessed = Promise.all(
	...
	res.filter((i) => i !== null && !i.draft).sort((a, b) => (b.date.getTime() > a.date.getTime() ? 1 : -1)),
);
...
export const Arti: Component = () => {
	...
	const [onhoverTitle, setOnhoverTitle] = createSignal<number | undefined>();
									...
									return (
										<>
											...
											<Show
												when={
													prevYear !== theY ||
													(prevSeason !== undefined && prevSeason !== getSeason(attr().date))
												}
											>
													...
													<div class="flex justify-end">
														...
														>
															<A
																...
																href="/taxonomy"
																onClick={() => {
																	let fstCat = attr().categories?.at(0);
																	if (fstCat) {
																		setTaxoJump({
																			id: fstCat,
																		})
															...
															>
																{attr().categories?.at(0) as string}
																<span class="block max-w-0 group-hover:max-w-full transition-all duration-350 h-px bg-sprout-500"
																	classList={{
																		"max-w-full": idx === onhoverTitle()
																	}} />
															</A>
														</Show>
													</div>
