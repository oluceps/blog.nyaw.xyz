...
const Search: Component = () => {
  ...
  const [idx, setIdx] = createSignal<Index<Record<string, any>>>()
  ...
  try {
    const client = new MeiliSearch({
      ...
      requestConfig: {
        headers: {
          // @ts-ignore
          Authorization: `Bearer ${import.meta.env.VITE_MS_KEY_RO}`
        },
      },
    });
    setIdx(client.index('blog'))
    idx()?.updateDisplayedAttributes(["path", "date", "title", "description"])
  } catch (e) {
    console.log(e);
    setUnexp(true);
  }
  ...
  createEffect(async () => {
    if (inp()) {
      ...
      try {
        const search = await idx()?.search(inp());
