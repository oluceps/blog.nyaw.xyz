...
const components = {
	...
	pre: (props: ParentProps) => {
		...
		return (
			<div class="relative group w-full">
				<pre
					{...props}
					class="w-full border bg-[#f9f9f9] px-4 py-2.5 overflow-auto scrollbar scrollbar-rounded"
					ref={setCodeBlockRef}
				>
					{props.children}
				</pre>
		...
		onMount(() => {
			if (childRef && childRef()?.parentElement) {
				...
				if (childRef()!.parentElement?.tagName != "PRE") {
					setInlineCodeClass("bg-sprout-100 px-1 text-sprout-950 rounded-sm inline flex-none font-medium leading-tight max-w-full whitespace-normal break-all");
				}
			}
		});
	...
	ol: (props: ParentProps) => (
		<ol {...props}
			class="pl-6 mb-2 list-decimal decoration-sprout-300 marker:text-sprout-400">
			{props.children}
		</ol>
	),
};
