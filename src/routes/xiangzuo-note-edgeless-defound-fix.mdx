...
确认没有复位的功能后，我左划右划了几分钟无果。我尝试导出pdf或图片但是只能导出当前显示的区域，显然是一片空白。也尝试合并笔记看能不能重置某个一骑绝尘的坐标数值，但是合并只能分页。
...
它们的储存格式包含肉眼可见的json字符串（以 UTF-8 解析），打开之后全局搜索 `offsetX` `offsetY` `scaleX` `scaleY` （可能都存在不止一个），然后把数值全部改成0。
