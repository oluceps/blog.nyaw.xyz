...
在部署端或者被部署端**使用 admin identity 解密**，并将 **使用host identity (pub) 加密的凭据** 在被部署端被 systemd 或者activation script 启动，使用 host identity 解密。

其中解密再加密的流程普遍称为 `rekey`
...
见 [wiki](https://nixos.wiki/wiki/Comparison_of_secret_managing_schemes#:~:text=a%20blog%20entry-,Comparison,-In%20case%20this)
