...
凭据管理工具通用的流程，用户在本地通过**admin identity (pub)** 加密并储存凭据，部署时或部署前（取决于各个工具的实现方式）**使用 admin identity 解密，并将使用host identity (pub) 加密的凭据**传输至远端host，对面的service 或者activation script 进行解密、链接。
