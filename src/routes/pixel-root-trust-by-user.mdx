...
以上命令仅为方便起见提供。avbroot 与任何标准 PKCS#8 编码的 4096 位 RSA 私钥和 PEM 编码的 X509 证书兼容，如 openssl 生成的那样。

如果丢失了 AVB 或 OTA 签名密钥，将无法签署新的 OTA zip 文件。您将需要生成新的签名密钥并重新解锁引导加载程序（触发数据擦除）。
...
这里是为了安装KernelSU，没需要就跳过本节。对应的KMI版本参照 [Android 文档](https://source.android.com/docs/setup/build/building-pixel-kernels?hl=zh-cn) 和 [KernelSU 文档](https://kernelsu.org/guide/installation.html#kmi) 来选择。
...
4. 从修补过的 OTA 中提取与原始版本不同的分区镜像。
    ...
    如果您希望为了安全起见提取并刷入所有操作系统分区，可以传入 `--all` 参数。
...
5. 刷入提取的分区镜像。
    ...
    请注意，这只刷入了操作系统分区。由于 fastboot 的限制，bootloader 和 modem/radio 分区未被更改。如果它们尚未更新或不确定，请在 fastboot 完成后，按照 [更新部分](#updates) 的步骤刷入修补后的 OTA。一旦进行 OTA 刷入，始终可以确保所有分区都是最新的。

    对于 Pixel 设备，您也可以通过从工厂镜像运行 `flash-base.sh` 来更新 bootloader 和 modem。

6. 在从 fastbootd 重启到 bootloader 后，设置自定义 AVB 公钥。
