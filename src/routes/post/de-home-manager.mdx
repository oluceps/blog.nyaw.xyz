...
description: You may actually don't need it
draft: false
toc: true
hideLevel: 10
...
For a nixos user includes me, home-manager may already introduced at the same time as the config created, for managing user-spec configs. But it has really poor maintainence<C>maintenance</C> and code quality, it does produced few bugs several months I use it <C>it has produced a few bugs over the several months I've been using it</C>
...
If you already <C>'ve had</C> have a deployed home-manager configuration, which linked <C>that links</C> things to your $HOME/.config directory, you may want to use this script to collect them all.
