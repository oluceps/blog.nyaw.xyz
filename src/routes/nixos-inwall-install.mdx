...
description: 即使身处果壳之中，仍自诩无限空间之王
categories:
  - 记录
tags:
  - nixos
...
首先， [flake](https://wiki.nixos.org/wiki/Flakes) 是一种面向未来的实验性方案，作用是更好地锁定版本以支持可复现。它通过 flake inputs 获取 nixpkgs 等仓库的源码，起到和channel近似的功能。
...
<Emph type="note">比较古旧的 nix中间连着hyphen的命令（如`nix-shell`, `nix-build`，但不包括带 `--flake` 参数的`nixos-rebuild`）都会在执行时请求 nix channel，
而新的实验性的nix command基本没有channel这一层抽象，都可以々某种方式直接指定仓库的url地址（如 `nix build`，`nix develop`，`nix profile`）。</Emph>
