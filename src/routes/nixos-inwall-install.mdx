...
+ 构建自带翻墙软件的livecd（不推荐）
  ...
  这个方法相比从镜像下载软件没有更方便，而且要求预先存在一个nix环境和正常的网络以构建livecd。你可以参考[wiki](https://wiki.nixos.org/wiki/Creating_a_NixOS_live_CD)和[我的livecd配置](https://github.com/oluceps/nixos-config/tree/3bc3d7215d43e66f8f8bdfed42b7d2cb087ca672/hosts/bootstrap)构建自带翻墙设施的image，flake下使用以下命令构建：
...
nixpkgs已经包含大量相关工具和设施，相见[tracking issue](https://github.com/NixOS/nixpkgs/issues/287770)。使用 [nix search](https://search.nixos.org/options?) 搜索相关选项并开启服务。如果你有能力，请尽量使用无图形界面的工具，因为GUI工具相比其实际生效的功能部件并没有提高更多效率，并且容易出现问题<C>仅在nixos的语境下，如 clash-verge 的GUI选项并不是完全生效</C>。
