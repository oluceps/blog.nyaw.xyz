...
```bash
# if pr in common branches ?
curl https://blog.nyaw.xyz/api/pr-stat?pr=11451
```

```bash
# if pr in specific branches ?
# [!code word:branch]
...?pr=11451&branch=nixos-24.11&branch=staging-next
```

```bash
# if pr in, specific repo, and branch
# [!code word:repo=daeuniverse/flake.nix]
...?repo=daeuniverse/flake.nix&pr=116&branch=unstable'|jq
```
...
## Query pr in other repo

e.g.

```bash
// [!code word:repo=]
> curl 'https://blog.nyaw.xyz/api/pr-stat?repo=daeuniverse/flake.nix&pr=116&branch=unstable'|jq

{
  "title": "[cherry-pick] chore(daed): pin ref to 0.8.0",
  "state": "merged",
