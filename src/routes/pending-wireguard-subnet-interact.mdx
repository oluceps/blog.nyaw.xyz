...
<Emph type="warn">本文所述问题未解决，可用的解决方案见[这里](/wg-babel-dyn-routing)</Emph>

我有几个机器在公网，少数不在公网<C>即后文内网机器，处于无法穿透的CGNAT下</C>，我想让在公网的机器实现 full mesh 互联，不在公网的通过公网机器进行中转实现互相连接。由于是一次性工作所以忽略掉配置复杂度。
