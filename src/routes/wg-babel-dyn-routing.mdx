...
这里是之后迁移的bird配置，用以替代babeld，示例没有做太多过滤。
...
+ 我所suffering的nat全都是 Independent mapping<C>NAT对每个对外连接单独分配IP:PORT</C>, Port depended filtering<C>只允许来自内部主机最初通讯的IP:PORT返回的流量。</C> 所以没有打洞成功的可能，白费了它的功能
+ 我的小内网没有ACL的需求
+ 不好和bird集成以便未来的路由拓展
+ go
