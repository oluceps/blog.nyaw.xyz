...
但是我折腾傻了也没搞明白这个wg到底怎么处理，在群友的建议下<C>特别感谢@Moraxyc</C>直接改造成了动态路由的 Overlay Network，Still based on wireguard.

为什么依然用wireguard而不是基于IPSec的 [ranet](https://github.com/NickCao/ranet) 或者同时支持 IPSec 和 Wireguard 的 [rait](https://gitlab.com/NickCao/RAIT/) 呢？
因为没有合规性和非常极端的安全要求所以不想用比较古老而且性能稍微弱点的IPSec；
因为不喜欢go所以不想选择后者（主要是它们两个文档对于不熟悉IPSec的人来说太稀缺了<C>虽然说本文描述的方案资料也是相当少qwq</C>）。

但是呢 Wireguard 它不支持 [Multicast](https://en.wikipedia.org/wiki/Multicast) 而且我们使用 [babel](https://en.wikipedia.org/wiki/Babel_(protocol)) 进行路由，
所以不能采用 Wiregurad 本身基于目的地址的端口复用[^1]<C>ranet作为rait的重写不支持wg也是出于这个原因</C>，
于是就只能每个 Wireguard Interface 单个 Peer，那么如果要构建类 full mesh 的网络就需要每个 node 上存在 `nodes - 1` 个interface [^2]。
庆幸的是我们用nixos进行配置，一切可以优雅而高效地进行而无需手动创建和定义接口路由。

[网络重构的全部更改](https://github.com/oluceps/nixos-config/pull/42)

# 配置基础

所以要做的就是在每个node上创建 `n - 1` 个具有唯一 peer 的 wireguard interface，然后再给它们配置地址使其互相一一连接，再把 babel 跑起来；
但是这些个地址有点讲究，babel要求 [IPv6 link local address](https://en.wikipedia.org/wiki/Link-local_address)才能运行，
但是Wireguard本身是不具备ll addr的，那么我们就需要手动配置一下。

回顾一下nixos上配置Wireguard，其实基本符合 systemd-networkd 的原有配置格式。分为 `netdevs` 和 `networks` 配置：
...
```
`Scope = "link"` 的 address 就是 link local addr 了（其实不写它也会自动标注成 ll, 因为 `fe80::`），这是必需的地址，其他的可以配一两个IPv4/v6地址，我这里只配了v6。
...
<Emph type="tips">
据我所知有些人在配置addresses中的参数时，需要加入对端的对应地址作为Peer参数才能生效，但在我的实践中**只有**删掉这个参数Babel才能正常处理路由。如果后期路由调试遇到问题请优先关注这个细节，具体导致差异的配置暂时未知。
</Emph>

tips: 上文的 link local address 可以用以下nushell脚本生成，或者使用参考[^2]的python脚本。

```nushell
echo $"fe80::(random binary 8 | encode hex --lower | split chars | window 4 --stride 4 | each {str join} | str join ':')"
```
...
需要注意本地的每个参与组网的wg interface都得写上，具体配置见[doc](https://www.irif.fr/~jch/software/babel/babeld.html)。

上文的 `router-id` 每个机器都唯一，一般查一下mac地址填上就好了，推荐手动配置。也可以用如下生成:

```nushell
echo $"(random binary 6 | encode hex --lower | split chars | window 2 --stride 2 | each {str join} | str join ':')"
```
...
基于我的[网络结构](https://github.com/oluceps/nixos-config/blob/9959b787aa023c30744c9f906fdb1b3e35631e33/hosts/sum.toml#L1-L70)所算出来的大概长这样：
...
接着完成最后一块拼图，即实现wg接口和地址配置的自动化。

## 生成

[Nix实现](https://github.com/oluceps/nixos-config/blob/9959b787aa023c30744c9f906fdb1b3e35631e33/repack/plugIn.nix)，
由于我的配置还涉及反审查和NAT下的机器的自动处理，可能会稍微复杂一点（当时填了半天真值表）。

回顾我们已经得到的信息，

+ peer 双端的hostname<C>在我的实现中用以索引预计算的配对端口信息，和拼接私钥名</C>
+ peer双端端口<C>康托尔配对得出</C>
+ 双端密钥
+ 双端内网地址
+ peer的Endpoint
